...
const MobileBook: React.FC = () => {
  ...
  return (
    <div className="relative w-full flex flex-col items-center justify-center py-6 px-4">
      {/* Контейнер карусели */}
      <div className="relative w-full max-w-[340px] h-[480px] overflow-hidden">
        <AnimatePresence initial={false} custom={direction} mode="wait">
          <motion.div
            ...
            className="absolute inset-0 rounded-2xl shadow-2xl overflow-hidden touch-pan-y"
          >
            <MobilePageContent page={page} index={currentPage} />
          </motion.div>
        </AnimatePresence>
        ...
        {currentPage > 0 && (
          <button
            onClick={() => paginate(-1)}
            className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full shadow-lg flex items-center justify-center z-20 active:scale-90 transition-transform"
          >
        ...
        {currentPage < pages.length - 1 && (
          <button
            onClick={() => paginate(1)}
            className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full shadow-lg flex items-center justify-center z-20 active:scale-90 transition-transform"
          >
      ...
      {/* Подсказка */}
      <div className="mt-4 flex items-center gap-3 text-primary/50 text-[10px] uppercase tracking-[0.3em]">
...
// Контент мобильной страницы
const MobilePageContent: React.FC<{ page: typeof pages[0]; index: number }> = ({ page, index }) => {
  const isCover = index === 0;

  if (isCover) {
    return (
      <div className="h-full w-full bg-[#2d2418] p-6 flex flex-col justify-between items-center text-white relative">
        <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/leather.png')]" />
        
        <div className="w-full flex justify-center items-center opacity-50 relative z-10 pt-2">
          <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-[#ec9213]">{page.est}</span>
        </div>

        <div className="flex flex-col items-center relative z-10 flex-1 justify-center">
          {/* Logo */}
          <div className="w-32 h-24 mb-4 flex items-center justify-center opacity-90">
  ...
  return (
    <div className="h-full w-full bg-[#fdfaf2] p-6 flex flex-col justify-center items-center text-center relative">
      <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/handmade-paper.png')]" />
      
      <div className="text-primary/40 mb-6 relative z-10">
        <span className="material-symbols-outlined text-6xl">{page.icon}</span>
      </div>
      
      <h3 className="font-serif text-2xl font-bold mb-4 text-dark tracking-wide uppercase leading-tight relative z-10">
        {page.title}
      </h3>
      
      <div className="w-16 h-px bg-primary/40 mb-4 relative z-10" />
      
      <p className="text-gray-700 text-base leading-relaxed mb-6 font-serif italic relative z-10 max-w-xs">
        {page.desc}
      </p>
      
      <button className="relative z-10 bg-primary text-white px-6 py-3 font-bold uppercase text-[10px] tracking-[0.3em] active:scale-95 transition-transform">
        {page.btn}
      </button>
      
      <p className="absolute bottom-4 text-[10px] text-primary/50 font-serif tracking-[0.2em] italic">
        {page.latin}
      </p>
    </div>
  );
...
// ============================================
// DESKTOP ВЕРСИЯ - 3D КНИГА
// ============================================
const DesktopBook: React.FC = () => {
  ...
  const handleToggle = (index: number) => {
    ...
    if (!isFlipped) {
      // Переворачиваем - проверяем что все предыдущие перевёрнуты
      for (let i = 0; i < index; i++) {
        if (!flippedPages[i]) return;
      }
      newFlipped[index] = true;
    } else {
      // Возвращаем - проверяем что все следующие не перевёрнуты
  ...
  return (
    <div className="relative w-full flex flex-col items-center justify-center py-4 md:py-10 select-none">
      <div 
        className="relative w-[420px] h-[580px] md:w-[480px] md:h-[650px] lg:w-[520px] lg:h-[700px]"
        style={{ perspective: '2500px' }}
      >
        {/* Задняя статичная страница */}
        <div 
          className="absolute inset-0 bg-[#fdfaf2] rounded-r-2xl shadow-inner border-l-8 border-gray-200 overflow-hidden"
          style={{ zIndex: 1 }}
        >
        ...
        <DesktopPage 
          ...
          content={pages[2]}
          zIndex={getZIndex(2, flippedPages[2], 10)}
        />
        <DesktopPage 
          index={1} 
          ...
          content={pages[1]}
          zIndex={getZIndex(1, flippedPages[1], 20)}
        />
        <DesktopPage 
          index={0} 
          ...
          content={pages[0]}
          zIndex={getZIndex(0, flippedPages[0], 30)}
          isCover={true}
        />
      </div>
      ...
      <div className="mt-14 flex items-center gap-5 text-primary/50 text-[11px] uppercase tracking-[0.5em] font-black italic">
        ...
        <span>Интерактивное издание · Кликните для перелистывания</span>
      </div>
    </div>
  );
};
...
interface DesktopPageProps {
  ...
  isFlipped: boolean;
  onToggle: () => void;
  content: typeof pages[0];
  zIndex: number;
  isCover?: boolean;
}

const DesktopPage: React.FC<DesktopPageProps> = ({ index, isFlipped, onToggle, content, zIndex, isCover }) => {
  return (
    <motion.div
      ...
      onClick={onToggle}
      whileHover={{ 
        rotateY: isFlipped ? -170 : -8,
        transition: { duration: 0.3 }
      }}
    >
      {/* ЛИЦЕВАЯ СТОРОНА */}
      <div 
        className={`absolute inset-0 rounded-r-2xl shadow-2xl flex flex-col border-l-2 border-gray-200/20 ${
          isCover ? 'bg-[#2d2418] border-r-4 border-[#ec9213]/40' : 'bg-[#fdfaf2]'
        }`}
        style={{ 
      ...
      {/* ОБРАТНАЯ СТОРОНА */}
      <div 
        className="absolute inset-0 rounded-l-2xl bg-[#f4f0e6] shadow-2xl border-r-8 border-gray-300/30 overflow-hidden"
        style={{ 
      ...
      >
        <BacksideContent content={content} />
      </div>
    </motion.div>
  );
};
...
// Контент обложки
const CoverContent: React.FC<{ content: typeof pages[0] }> = ({ content }) => (
  <div className="h-full w-full p-10 flex flex-col justify-between items-center text-white relative overflow-hidden">
    ...
    <div className="flex flex-col items-center relative z-10 mt-8">
      ...
      <div className="w-16 h-px bg-[#ec9213]/60 mb-6" />
...
// Контент обычной страницы
const PageContent: React.FC<{ content: typeof pages[0] }> = ({ content }) => (
  <div className="h-full p-10 sm:p-14 flex flex-col justify-center items-center text-center relative overflow-hidden">
    ...
    <button className="relative z-10 border-2 border-primary/30 text-dark px-10 py-5 font-black uppercase text-[11px] tracking-[0.4em] hover:bg-primary hover:text-white transition-all duration-500">
      {content.btn}
    </button>
...
const BacksideContent: React.FC<{ content: typeof pages[0] }> = ({ content }) => (
  <>
